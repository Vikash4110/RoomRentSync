...
//     if (decoded.role !== "landlord") throw new Error("Invalid role");
//     req.user = decoded;
//     next();
//   } catch (error) {
...
// module.exports = { landlordMiddleware, validate, errorMiddleware };

const jwt = require("jsonwebtoken");
const { z } = require("zod");

// Middleware allowing both "client" and "landlord" roles
...
// Middleware restricting access to "landlord" role only
const landlordOnlyMiddleware = async (req, res, next) => {
  const token = req.header("Authorization")?.replace("Bearer ", "");
  if (!token) return res.status(401).json({ error: "No token provided" });
...
module.exports = { authMiddleware, landlordOnlyMiddleware, validate, errorMiddleware };